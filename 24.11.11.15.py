...
nouns = {
    "food": ["pizza", "pasta", "burger", "salad", "soup", "pasta", "cake", "ice cream"],
...
def generate_sentence(tense="present"):
    ...
    obj = weighted_random_choice(nouns[obj_category], word_probabilities["nouns"])
    ...
    # Apply tense
    sentence = apply_tense(subject, verb, tense) + sentence[1:]  # Adjust for tense (e.g., "I eat" -> "I eats")
    
    return sentence

# Tense application function (e.g., present tense)
def apply_tense(subject, verb, tense="present"):
    if tense == "present":
